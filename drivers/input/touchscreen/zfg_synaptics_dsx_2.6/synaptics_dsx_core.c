		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	int gesture_x_distance;  // gesture_swipe
	int gesture_y_distance;  //gesture_swipe
    int gesture_direction;   //gesture_swipe
	...
	if (rmi4_data->suspend && rmi4_data->enable_wakeup_gesture) {
		...
		gesture_type = rmi4_data->gesture_detection[0];
                gesture_x_distance = rmi4_data->gesture_detection[1];  //gesture_swipe
		gesture_y_distance = rmi4_data->gesture_detection[2];  //gesture_swipe
		gesture_direction = rmi4_data->gesture_detection[3];  //gesture_swipe

        printk("syna gesture_type  = %d,%d,%d %d\n",gesture_type,gesture_x_distance,gesture_y_distance,gesture_direction);
		//printk("syna gesture_type  = %c,%d,%d %d\n",gesture_type,gesture_x_distance,gesture_y_distance,test_para);

		if (gesture_type && gesture_type != F12_UDG_DETECT) {
            if(gesture_type == 3)
            {
                printk("syna,it's double-click\n");
			input_report_key(rmi4_data->input_dev, KEY_WAKEUP, 1);
			input_sync(rmi4_data->input_dev);
			input_report_key(rmi4_data->input_dev, KEY_WAKEUP, 0);
			input_sync(rmi4_data->input_dev);
            ...
            {
                printk("syna,it's swipe\n");
                switch(gesture_direction){
				case 0x1:
				case 66:
                {
                    printk("syna,it's left swipe\n");
                    input_report_key(rmi4_data->input_dev, KEY_GESTURE_LEFT, 1);
			        input_sync(rmi4_data->input_dev);
			        input_report_key(rmi4_data->input_dev, KEY_GESTURE_LEFT, 0);
			        input_sync(rmi4_data->input_dev);
					break;
                }
                case 0x2:
				case 65:
                {
                    printk("syna,it's right swipe\n");
                    input_report_key(rmi4_data->input_dev, KEY_GESTURE_RIGHT, 1);
			        input_sync(rmi4_data->input_dev);
			        input_report_key(rmi4_data->input_dev, KEY_GESTURE_RIGHT, 0);
			        input_sync(rmi4_data->input_dev);
					break;
                }
				default:
...
static void synaptics_rmi4_set_params(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (rmi4_data->f11_wakeup_gesture || rmi4_data->f12_wakeup_gesture) {
		...
		set_bit(KEY_GESTURE_RIGHT, rmi4_data->input_dev->keybit);
		set_bit(KEY_GESTURE_LEFT, rmi4_data->input_dev->keybit);
